...
public class LetterManageImpl extends AbstractService implements LetterManage{
	...
	public JSON getAllLetter(int userId) {
		String sql = "SELECT t1.F01,t3.F02,t1.F04,t1.F05,t1.F06,t2.F02 FROM T60 AS t1 LEFT "
				+ "JOIN T61 AS t2 ON t1.F01 = t2.F01 LEFT "
				+ "JOIN T10 AS t3 ON t1.F02 = t3.F01 WHERE t1.F02 = ? AND t1.F07 != 'S'";
		...
		try {
				...
				public void execute(ResultSet rs) {
					...
					try {
						while(rs.next()) {
							...
							json.put("timestamp", rs.getTimestamp(3));
							json.put("readed", rs.getString(4));
			...
			sql = "SELECT COUNT(F01) FROM T60 WHERE F02 = ?";
			this.select(conn, sql, new SelectExecutor() {
